...
- `GALLERY`：从八种形态中稳定抽取三种作为本次滚轮序列并提前预载，同时允许箭头和键盘浏览全部八种；始终只显示当前视频，其余抽样视频保持隐藏待播。
- `CHARACTER`：全身形象与脸部、服装、背面、不对称袜鞋四组设计细节。
- `GAZE`：通过鼠标、触摸或方向键控制视频时间轴，让夜希的目光跟随访问者。
- `CREATOR`：创作者资料与六张可交互的夜希视觉卡片。
- `LINKS`：项目展示、外部入口目录，以及返回首页入口。
...
├─ skills/immersive-ip-website/    # 与仓库共同版本化的沉浸式网站 Skill
├─ src/
│  ├─ app/
...
仓库包含构建网站所需的源码、锁定依赖、运行时图片、视频、音频和 Skill。服务器克隆仓库后直接安装依赖并构建，不需要从本地电脑另行复制媒体；`dist/` 是服务器生成的生产目录，因此不提交到 Git。

```bash
git clone https://github.com/CheshireMew/Nyxie-Web.git
cd Nyxie-Web
npm ci
NYXIE_SITE_URL="https://example.com/nyxie/" npm run build
```

将 Nginx、Apache、对象存储或静态托管平台的站点根目录指向仓库中的 `dist/`。以后更新使用：
...
服务器需要 Node.js `22.12+`。如果网站部署在域名根路径，把 `NYXIE_SITE_URL` 写成以 `/` 结尾的完整根地址；部署在子目录时写入对应子目录地址。这个变量只负责 canonical 和分享元数据，页面资源本身使用相对路径，根路径和静态子目录都可以部署。
...
`check` 包含 TypeScript、纯逻辑聚焦测试、架构边界守卫和媒体清单验证，并拒绝循环依赖、旧导航协议、重新膨胀的入口文件和任何未在清单声明的部署文件。浏览器验收脚本需要网站已在 `4173` 端口运行，并且本机已有监听 `9222` 端口的 Chromium DevTools；它会验证静态回退、深链接与历史遍历、真实媒体加载与失败重试、章节滚动、键盘/触摸入口、响应式布局和减少动态效果路径。报告与截图写入 `artifacts/browser-acceptance/`。

`build` 生成可部署的 `dist/` 静态目录，并自动执行 `validate:dist`：按照媒体清单逐一比对生产副本、拒绝未声明媒体和开发期入口、检查相对 JS/CSS/静态资源。`base: "./"` 允许部署在域名根路径或静态子目录。CI 构建后会分别从根路径和 `/dist/` 静态子目录启动真实产物，再运行同一套浏览器验收。
...
启动屏阶段只请求首屏锚点和当前首屏视频；首屏就绪后预热 Gallery 本次抽样的三张海报和三段固定滚轮视频，确保进入章节后切换不再等待临时下载，后续只预热当前章节的下一章。BGM 仍在用户主动开启后才设置音频源。媒体请求失败会保留最后可用画面或静态图，并在原控制位置提供真实重试。

`public/assets/` 是必须随仓库推送的版本化运行时媒体边界，`src/content/mediaManifest.json` 是路径清单；生产构建会逐一复制并校验这些文件。`skills/` 同样属于仓库正式内容，但不进入网站的 `dist/`。静态角色素材的再生成方式和视频、音频的更新规则见 `scripts/MEDIA_PIPELINE.md`。
